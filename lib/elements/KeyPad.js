...
const KeyPad = forwardRef(function KeyPad(
  { displayRule, position, validation, label, confirm, cancel, keyValid, value, update, sync , incrementNum,decimal,mask},
  ref
) {
  // @ts-ignore
  ...
  function increment (){
    const b = parseFloat(inputValue) + incrementNum;
    setInputValue(b.toFixed(decimal));
  };
  const decrement = () =>{
    const b = parseFloat(inputValue) - incrementNum;
    setInputValue(b.toFixed(decimal));
  ...
  useEffect(() => {
    if (keyboard.keyDownEvent) {
      ...
      } else if(!mask && ['ArrowUp'].includes(keyboard.keyDownEvent.key)){
        increment();
      } else if(!mask && ['ArrowDown'].includes(keyboard.keyDownEvent.key)){
      decrement();
      } else {

...
const KeyPad = forwardRef(function KeyPad(
  { displayRule, position, validation, label, confirm, cancel, keyValid, value, update, sync , incrementNum,decimal},
  ref
) {
  // @ts-ignore
  ...
  useEffect(() => {
    if (keyboard.keyDownEvent) {
      ...
      } else if(['ArrowUp'].includes(keyboard.keyDownEvent.key)){
        increment();
      } else if(['ArrowDown'].includes(keyboard.keyDownEvent.key)){
      decrement();
      } else {
